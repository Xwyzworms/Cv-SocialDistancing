...
YOLOV3_LABELS_PATH = os.getcwd() + "./coco.names"
YOLOV3_CFG_PATH = os.getcwd() + "./yolo.cfg"
YOLOV3_WEIGHTS_PATH = os.getcwd() + "./yolov3.weights"
VIDEO_PATH = os.getcwd() + "./vid3.mp4"
OUTPUT_PATH = os.getcwd() + "./output.avi"
SAFE_DISTANCE = 60
np.random.seed(99)
...
neural_net = cv2.dnn.readNetFromDarknet(YOLOV3_CFG_PATH, YOLOV3_WEIGHTS_PATH) # load Weights Dari yolo
layer_names = neural_net.getLayerNames()
layer_names = [layer_names[i[0] - 1] for i in neural_net.getUnconnectedOutLayers()] #GEt Output layernya
...
while True:
    ...
    blob = cv2.dnn.blobFromImage(frame, 1 / 255.0, (416, 416), swapRB=True, crop=False) # Init inputan
    neural_net.setInput(blob)  # masukin inputan ke Neural Net

    start_time = time.time()
    layer_outputs = neural_net.forward(layer_names) # Lakuin prediksi
    end_time = time.time()
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            cv2.putText(frame, 'Ga Social distancing: {}'.format(count), (70, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 3)            
